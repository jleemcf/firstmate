...
- Under live PR enrichment, only a `candidate_prs` row matching a task's `recorded_prs` URL and carrying `merge_ready: true` promotes that PR to Captain's Call; an approved and green but conflicting or unknown-mergeability PR remains Underway for repair.
- Merge evidence ranks from a run or status claim, to a structured Done row, to the PR's own state. The strongest evidence owns the bucket, and every weaker contradiction is disclosed rather than trusted or silently dropped.
- A structured Done row outranks a leftover live-state claim and remains Recently Landed with the contradiction disclosed, while a verified open PR outranks a contradictory Done row and keeps the item Underway as `awaiting_landing`.
