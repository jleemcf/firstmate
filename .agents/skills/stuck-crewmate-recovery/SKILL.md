...
A pool slot refused because its `.fm-task-owner` marker names a task with no record was taken by a spawn that was killed before it published one; the refusal reports whether that spawn's ownership record still stands beside the state directory and names both files, and clearing them by hand after confirming the slot is idle is the only recovery, because no teardown exists for a task that was never recorded.
A fresh spawn of that same task id refuses for the same reason until those files are gone, naming the unresolved spawn generation and the worktree it took, so resolve the stranded slot first and then spawn the id again.
