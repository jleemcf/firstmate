...
The worker serves one lane per staged home: jobs for the same home run strictly first-in-first-out in staging order while different homes' lanes run concurrently, so one home's long job never delays another home's commands.
...
A caller that disconnects or times out before its job completes cancels it instead of abandoning it: the worker skips a cancelled queued job, stops a running cancelled job's process group, and cleans up the record, so retries never convoy behind abandoned work.
