...
A pooled crewmate slot is a durable lease held under the task id, so it is never handed to a second task or pruned while that task's record exists, and teardown is what releases it.
