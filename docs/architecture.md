...
Bearings treats only structured Done rows as landing evidence on its default path, so a terminal ship without that evidence remains Underway with its merge state explicitly unverified.
