...
A record that names a spawn generation is one the marker era wrote, so an absent `.fm-task-owner` on a path that still exists refuses by task id, path, and missing marker rather than falling back to the task branch or project membership; only a record that carries no spawn generation line at all is old enough for those fallbacks to still mean anything.
A duplicated, empty, or unreadable spawn generation is not that record: it named a generation and then said nothing usable about it, so it refuses on the same rule rather than being read as pre-marker.
