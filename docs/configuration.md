...
FM_CHROME_BRIDGE_TIMEOUT=20   # 1..120 seconds allowed for one chrome-devtools-axi call during task teardown; invalid or out-of-range values use 20 and 120 respectively. Only a task whose launcher recorded a bridge start makes any browser call at all, so an unused task never waits on the tool
