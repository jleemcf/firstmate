...
`chrome-devtools-axi` carries one extra contract beyond being installed: every crewmate and scout is launched with its own non-default `CHROME_DEVTOOLS_AXI_SESSION`, and teardown reclaims that task's bridge by asking the resolved `chrome-devtools-axi` about that exact session name.
